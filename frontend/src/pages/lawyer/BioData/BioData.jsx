...
import { profileSetup } from '../../../services/lawyer.service';
...
const BioData = () => {
    ...
    const { mutate, isPending: isLoading } = useMutation({
        ...
        onSuccess: () => {
            ...
            setUserDetails((prev) => ({...prev, bioDataProvided: true}));
            navigate("/");
        },
        onError: (err) => {
    ...
    const handleImageChange = (e) => {
        ...
        if (file) {
            ...
            setFilePreviews((prev) => ({ ...prev, profileImage: URL.createObjectURL(file) }));
            setErrors((prevData) => ({ ...prevData, [name]: "" }));
        }
    };
    ...
    const handleSubmit = (e) => {
        ...
        for (const key in fileData) {
            if (fileData[key]) {
                form.append(key, fileData[key], fileData[key].name);
            }
        }
...
                          <img src={filePreviews.profileImage} alt="Profile" className="w-full h-full rounded-full object-cover" />
                        ) : (
...
                                      {fileData[doc.name]?.type.startsWith('image/') ? (
...
                                      )}
                                      <p className="text-gray-300 text-sm truncate" title={fileData[doc.name]?.name}>
                                          {fileData[doc.name]?.name}
                                      </p>
                                  </div>
...
                          {doc.description && <p className="text-gray-500 text-xs mt-2">{doc.description}</p>}
                          {errors[doc.name] && <p className="text-red-500 text-sm mt-1">{errors[doc.name]}</p>}
                    </div>
                  ))}
                  </div>
                </div>
